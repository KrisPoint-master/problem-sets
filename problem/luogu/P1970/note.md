...
仔细读题后，发现最终寻找的的是交替式数列的元素的个数（包括头尾）
...
想念你01序列超绝 O(2 ^ n)复杂度吗？？？

对于 $100\%$的数据，$1 \le n \le {10}^5$，$0 \le h_i \le {10}^6$
...
$up(i)$ 表示 $ up(i) > up(i-1) $ 的摆动数列的个数
$down(i)$ 表示 $ down(i) < down(i-1) $ 的摆动数列的个数

则 $ \displaystyle \left\{\begin{array}{l} up_i = max(up_i,down_i + 1) ,\ h(i) > h(i-1) \\ down_i = max(down_i,up_i + 1) ,\ h(i) < h(i-1) \end{array}\right. $
