...
private:
    ...
    int _range_query(int node, int l, int r, int i, int j)
    {
        ...
        int ans = 0;
        if (ans_left != INT_MIN / 3)
            ans = (ans + (ll)ans_left) % MOD;
        if (ans_right != INT_MIN / 3)
            ans = (ans + (ll)ans_right) % MOD;
        return ans;
    }
};
...
int main(int argc, char **argv)
{
    int n, m;
    cin >> n >> m;
    LazySegmentTree sgt(n + 5);
    for (int i = 1; i <= n; i++)
    {
        int t;
        cin >> t;
        sgt.point_update_add(i, t);
    }
    ...
    for (int i = 0; i < m; i++)
    {
        ...
        if (flag == 1)
        {
            int x, y, k;
            cin >> x >> y >> k;
            sgt.range_update_add(x, y, k);
        }
        else
        {
            int x, y;
            cin >> x >> y;
            cout << sgt.range_query(x, y) << endl;
        }
    }
